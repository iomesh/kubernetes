...
func InitAuthTestSuite() storageframework.TestSuite {
	patterns := []storageframework.TestPattern{
		storageframework.CredentialDynamicPV,
	}
...
func (a *authTestSuite) DefineTests(driver storageframework.TestDriver, pattern storageframework.TestPattern) {
	...
	init := func() {
		...
		l.authInfo = authDriver.GetAuthInfo()
	...
	ginkgo.It("pod should attach pvc success using current authinfo", func() {
		init()
		defer cleanup()
		for _, paramValue := range l.scAuthParams {
			if strings.HasSuffix(paramValue, "secret-name") {
...
func createSecret(kubeClient clientset.Interface, namespace, name string, data map[string]string) error {
	secret := makeSecret(namespace, name, data)
...
func makeSecret(namespace, name string, data map[string]string) *v1.Secret {
	secret := &v1.Secret{
